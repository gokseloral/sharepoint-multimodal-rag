...
def _matches_metadata_filter(fields: dict, mf: "MetadataFilterConfig") -> bool:
    """Return ``True`` if *all* configured column=value pairs match.

    Comparison is case-insensitive for values; column names are used verbatim
    (they must match the internal SharePoint column name exactly).

    Args:
        fields: Dict of ``{column_name: value}`` from the SharePoint list item.
        mf: The MetadataFilterConfig to evaluate.
    """
    for column, expected in mf.filters:
...
class SharePointClient:
    ...
    def list_files(
        ...
        if metadata_filter and metadata_filter.enabled:
            columns = ",".join(col for col, _ in metadata_filter.filters)
